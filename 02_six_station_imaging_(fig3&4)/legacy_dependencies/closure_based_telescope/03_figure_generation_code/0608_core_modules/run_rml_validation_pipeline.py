...
def adam_amp_closure_rml(
    ...
    for iteration in range(1, ADAM_ITER + 1):
        ...
        score = float(values["amp_chi2"] + values["phase_chi2"])
...
def residual_selection_score(residuals: dict[str, float], internal_objective: float) -> float:
    ...
    by physical reduced-chi2 diagnostics, with the original objective retained
    only as a small tie-breaker.
    """
    amp_chi2 = max(float(residuals["amp_reduced_chi2"]), 1.0)
    phase_chi2 = max(float(residuals["phase_reduced_chi2"]), 1.0)
    return float(np.log10(amp_chi2) + np.log10(phase_chi2) + 0.01 * internal_objective)
