...
JSON_OUT = TABLE_DIR / "closure_loop_rms_snr1_table_gauge_marginalized.json"
CSV_OUT = TABLE_DIR / "closure_loop_rms_snr1_table_gauge_marginalized.csv"
HAWAII_JSON_OUT = TABLE_DIR / "closure_loop_rms_snr1_hawaii_top4_plus3_ngc4151_gauge_marginalized.json"
HAWAII_CSV_OUT = TABLE_DIR / "closure_loop_rms_snr1_hawaii_top4_plus3_ngc4151_gauge_marginalized.csv"
TEX_OUT = PDF_DIR / "closure_loop_rms_snr1_note_gauge_marginalized.tex"
PDF_OUT = PDF_DIR / "closure_loop_rms_snr1_note_gauge_marginalized.pdf"
...
def compute_loop_rows() -> dict:
    ...
    for index, (loop_def, w_loop) in enumerate(zip(loop_defs, w_basis.T), start=1):
        ...
        rows.append(
            add_resolvability_flags(
                ...
                "gain_intrinsic_ideal_over_unsplit_edge": float((sigma_edge / sigma_ideal) / split_snr_factor),
                "gain_workpoint_limited_over_edge": float(sigma_edge / sigma_cfi),
                "gain_intrinsic_workpoint_limited_over_unsplit_edge": float((sigma_edge / sigma_cfi) / split_snr_factor),
    ...
    payload = {
        ...
        "gain_factorization": (
            "G_QFI and G_CFI compare against simultaneous split edge-first. "
            "G_joint=G_QFI/sqrt(N-1) removes the common no-splitting factor."
        ),
...
def table_lines(rows: list[dict]) -> list[str]:
    lines = [
        r"\begin{longtable}{r l r r r r r r r r c}",
        r"\caption{Per-loop closure phase RMS and gains at global SNR gain 1. RMS values are in radians. $G_{\rm QFI}$ is the total gain over simultaneous split edge-first, while $G_{\rm joint}=G_{\rm QFI}/\sqrt{N-1}$ removes the common no-splitting factor. The Res. column marks strategies with $\sigma_\Phi<\pi/5$: E=edge, Q=direct QFI, C=direct CFI.}\\",
        r"\toprule",
        r"Idx & Loop & $\chi_2$ & $\chi_3$ & $\sigma_{\rm edge}$ & $\sigma_{\rm dir}^{\rm QFI}$ & $\sigma_{\rm dir}^{\rm CFI}$ & $G_{\rm joint}$ & $G_{\rm QFI}$ & $G_{\rm CFI}$ & Res. \\",
        r"\midrule",
        r"\endfirsthead",
        r"\toprule",
        r"Idx & Loop & $\chi_2$ & $\chi_3$ & $\sigma_{\rm edge}$ & $\sigma_{\rm dir}^{\rm QFI}$ & $\sigma_{\rm dir}^{\rm CFI}$ & $G_{\rm joint}$ & $G_{\rm QFI}$ & $G_{\rm CFI}$ & Res. \\",
    ...
    for row in rows:
        lines.append(
            ...
            rf"{latex_number(row['gain_intrinsic_ideal_over_unsplit_edge'])} & "
...
def write_tex(payload: dict, hawaii_payload: dict) -> None:
    ...
    med_gain_joint = float(np.median([row["gain_intrinsic_ideal_over_unsplit_edge"] for row in rows]))
    ...
    hawaii_med_gain_joint = float(
        np.median([row["gain_intrinsic_ideal_over_unsplit_edge"] for row in hawaii_rows])
    )
    ...
    lines = [
        ...
        r"\begin{document}",
        r"\begin{center}",
        r"{\Large Gauge-marginalized closure-loop phase RMS at unit SNR gain}\\[3pt]",
        r"{\normalsize Versioned correction: full edge-phase QFI with station-gauge nuisance removed}",
        r"\end{center}",
        ...
        (
            ...
            rf"Because edge-first simultaneously splits each station into $N-1$ outgoing baseline "
            rf"combiners, these total gains include the common no-splitting factor "
            rf"$G_{{\rm split}}=\sqrt{{N-1}}={payload['split_snr_factor']:.2f}$; "
            r"$G_{\rm joint}=G_{\rm QFI}/G_{\rm split}$ isolates the intrinsic three-mode SLD gain. "
        ...
        r"\paragraph{Summary.}",
        (
            rf"The median total ideal closure-first gain is $G_{{\rm QFI}}={med_gain_ideal:.2f}$; "
            rf"after removing the split factor, the median intrinsic joint gain is "
            rf"$G_{{\rm joint}}={med_gain_joint:.2f}$.  "
            rf"Finite working-point sensitivity gives median "
    ...
    lines.extend(
        [
            ...
            (
                ...
                rf"$G_{{\rm QFI}}={hawaii_med_gain_ideal:.2f}$, while the intrinsic "
                rf"$G_{{\rm joint}}={hawaii_med_gain_joint:.2f}$ after removing "
                rf"$\sqrt{{N-1}}={hawaii_payload['split_snr_factor']:.2f}$.  Finite workpoint sensitivity gives "
