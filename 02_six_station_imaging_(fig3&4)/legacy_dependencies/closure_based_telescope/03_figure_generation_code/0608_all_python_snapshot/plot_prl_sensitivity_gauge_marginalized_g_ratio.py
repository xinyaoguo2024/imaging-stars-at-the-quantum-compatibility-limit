...
def three_station_fisher_density(
    u: np.ndarray,
    *,
    station_etas: tuple[float, float, float],
    nus: tuple[float, float, float],
    strategy: str,
) -> np.ndarray:
    """Per-temporal-frequency-mode FI for closure phase using the explicit SLD model.

    The station efficiencies eta_i multiply both the one-click load and the
    coherent amplitude; false positives are independent mode-local occupancies.
    """
    eta = np.asarray(station_etas, dtype=float)
    ...
    for idx in np.ndindex(u.shape):
        ss = np.array(
            [float(s1[idx] + eps_dir[0]), float(s2[idx] + eps_dir[1]), float(s3[idx] + eps_dir[2])]
        )
        scale = max(float(np.mean(ss)), 1e-300)
        out[idx] = scale * cfi_tables.triangle_direct_fisher(
            float(g12[idx]) / scale,
            float(g23[idx]) / scale,
            float(g31[idx]) / scale,
            ss[0] / scale,
...
def gauge_marginalized_gain_grid(ratios: np.ndarray, *, g_ref: float = 0.5) -> tuple[np.ndarray, np.ndarray]:
    """Gauge-marginalized three-station gain and intrinsic joint-measurement part.

    The reference model has equal station loads s=1 and coherent amplitudes
    (g12, g23, g31) = (g_ref, r23*g_ref, r31*g_ref), so the axes are the
    physical coherent-amplitude ratios g23/g12 and g31/g12.
    """
    r23, r31 = np.meshgrid(ratios, ratios, indexing="xy")
    gain = np.empty_like(r23, dtype=float)
    intrinsic = np.empty_like(r23, dtype=float)
    s1 = s2 = s3 = 1.0
    ...
    for idx in np.ndindex(r23.shape):
        g12 = g_ref
        g23 = g_ref * float(r23[idx])
        g31 = g_ref * float(r31[idx])
...
def main() -> None:
    ...
    ax = axes[0]
    ratios = np.logspace(-2.0, 0.0, 121)
    r23, r31 = np.meshgrid(ratios, ratios, indexing="xy")
    ...
    reference_nu12 = 0.5
    reference_eta_u_over_epsilon = 50.0
    reference_g_over_s = reference_nu12 * reference_eta_u_over_epsilon / (reference_eta_u_over_epsilon + 1.0)
    gain, intrinsic_gain = gauge_marginalized_gain_grid(ratios, g_ref=reference_g_over_s)
    image = ax.pcolormesh(
        r23,
        r31,
        gain,
    ...
    ax.set_title(r"(a) closure gain")

    ax = axes[1]
    line_styles = {
        ("symmetric loop", "split"): ("#bb3e03", "--", "sym edge"),
        ("symmetric loop", "direct"): ("#bb3e03", "-", "sym direct"),
        ("asymmetric loop", "split"): ("#005f73", "--", "asym edge"),
        ("asymmetric loop", "direct"): ("#005f73", "-", "asym direct"),
    }
    reference_limits_30 = {}
    ...
    ax.set_ylabel(r"AB limit (SNR $=3$)")
    ax.set_title(r"(b) source reach")
    ...
    png = OUTDIR / "prl_closure_sensitivity_gauge_marginalized_g_ratio.png"
    pdf = OUTDIR / "prl_closure_sensitivity_gauge_marginalized_g_ratio.pdf"
    fig.savefig(png, dpi=260, bbox_inches="tight")
    fig.savefig(pdf, bbox_inches="tight")
    root_png = ROOT_FIGDIR / "prl_closure_sensitivity_gauge_marginalized_g_ratio.png"
    root_pdf = ROOT_FIGDIR / "prl_closure_sensitivity_gauge_marginalized_g_ratio.pdf"
    ...
    stats = {
        ...
        "heatmap_reference_nu12": float(reference_nu12),
        "heatmap_reference_eta_u_over_epsilon": float(reference_eta_u_over_epsilon),
        "heatmap_reference_g12_over_s": float(reference_g_over_s),
        "heatmap_axes": "g23/g12 and g31/g12 at s1=s2=s3; equals nu ratios for equal u_i, eta_i, epsilon_i",
    ...
    for name, cfg in scenarios.items():
        scenario_stats = {
            "station_efficiencies_eta1_eta2_eta3": list(cfg["station_etas"]),
    ...
    stats_path = OUTDIR / "prl_closure_sensitivity_gauge_marginalized_g_ratio_stats.json"
    stats_path.write_text(json.dumps(stats, indent=2) + "\n")
    (ROOT_FIGDIR / "prl_closure_sensitivity_gauge_marginalized_g_ratio_stats.json").write_text(
