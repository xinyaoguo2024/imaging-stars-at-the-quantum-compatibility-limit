...
LOOP_JSON = TABLE_DIR / "eight_station_ideal_loop_gain_loss020.json"
LOOP_TEX = TABLE_DIR / "eight_station_ideal_loop_gain_loss020.tex"
MISMATCH_JSON = TABLE_DIR / "eight_station_phase_frame_cfi_loss020.json"
MISMATCH_TEX = TABLE_DIR / "eight_station_phase_frame_cfi_loss020.tex"
...
def make_loop_gain_table(context: dict) -> dict:
    ...
    lines = [
        ...
        r"edge-first post-processing for that loop.  $G_{\rm CFI}$ includes the conservative",
        ...
        r"\hline\hline",
        r"Index & Loop & $\chi_2$ & $\chi_3$ & $\sigma_{\rm edge}$ & $\sigma_{\rm dir,CFI}$ & $G_{\rm CFI}$ & $G_{\rm loop}$ \\",
        r"\hline",
    ]
    for row in rows:
        lines.append(
            ...
            f"{latex_number(row['snr_gain_loop_cfi_scheduled'])} & {latex_number(row['snr_gain_loop_qfi'])} \\\\"
        )
