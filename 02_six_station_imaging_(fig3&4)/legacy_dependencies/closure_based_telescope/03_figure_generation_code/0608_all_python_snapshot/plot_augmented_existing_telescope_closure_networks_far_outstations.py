...
def maunakea_plus5_far_case(rng: np.random.Generator) -> aug.NetworkCase:
    ...
    return aug.NetworkCase(
        key="maunakea_plus5_far",
        title="Maunakea optical core + five far 5 m outstations",
        latitude_deg=center[0],
...
def ctio_plus_far_case(rng: np.random.Generator, n_added: int) -> aug.NetworkCase:
    wanted = {
        "CTIO / Blanco": 4.0,
        "Gemini South": 8.1,
        "SOAR": 4.1,
    ...
    case = aug.optimize_added_telescopes(
        ...
        max_target_g_lambda=80.0,
        rng=rng,
        n_trials=3900 if n_added == 3 else 4700,
    )
    return aug.NetworkCase(
        key=f"ctio_plus{n_added}_far",
        title=f"CTIO/Pachon/Rubin core + {n_added} far five-meter outstations",
        latitude_deg=center[0],
        center_latlon=center,
        telescopes=case.telescopes,
        hub_km=case.hub_km,
...
def main() -> None:
    rng = np.random.default_rng(aug.RNG_SEED + 12020)
    cases = [
        maunakea_plus5_far_case(rng),
        ctio_plus_far_case(rng, 3),
        ctio_plus_far_case(rng, 4),
    ]
    summary = {}
    for case in cases:
    ...
    summary_path = OUTFIG / "augmented_existing_telescope_far_outstations_summary.json"
