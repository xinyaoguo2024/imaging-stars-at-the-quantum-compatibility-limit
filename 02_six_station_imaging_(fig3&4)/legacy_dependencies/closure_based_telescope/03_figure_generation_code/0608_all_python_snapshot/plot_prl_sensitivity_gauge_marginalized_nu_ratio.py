...
def gauge_marginalized_gain_grid(ratios: np.ndarray, *, nu_ref: float = 0.3) -> tuple[np.ndarray, np.ndarray]:
    ...
    (nu12, nu23, nu31) = (nu_ref, r23*nu_ref, r31*nu_ref), so the axes are
    the physical visibility-amplitude ratios nu23/nu12 and nu31/nu12.
    """
    ...
    for idx in np.ndindex(r23.shape):
        g12 = nu_ref
        g23 = nu_ref * float(r23[idx])
        g31 = nu_ref * float(r31[idx])
...
def main() -> None:
    ...
    reference_nu = 0.3
    gain, intrinsic_gain = gauge_marginalized_gain_grid(ratios, nu_ref=reference_nu)
    image = ax.pcolormesh(
        r23,
        r31,
        gain,
    ...
    ax.set_xlabel(r"$r_{23}=\nu_{23}/\nu_{12}$")
    ax.set_ylabel(r"$r_{31}=\nu_{31}/\nu_{12}$")
    ax.set_title(r"(a) Visibility-ratio gain")
    ...
    png = OUTDIR / "prl_closure_sensitivity_gauge_marginalized_nu_ratio.png"
    pdf = OUTDIR / "prl_closure_sensitivity_gauge_marginalized_nu_ratio.pdf"
    fig.savefig(png, dpi=260, bbox_inches="tight")
    fig.savefig(pdf, bbox_inches="tight")
    root_png = ROOT_FIGDIR / "prl_closure_sensitivity_gauge_marginalized_nu_ratio.png"
    root_pdf = ROOT_FIGDIR / "prl_closure_sensitivity_gauge_marginalized_nu_ratio.pdf"
    ...
    stats = {
        ...
        "heatmap_reference_nu12": float(reference_nu),
        "heatmap_axes": "r23=nu23/nu12 and r31=nu31/nu12 for equal u_i, eta_i, epsilon_i",
    ...
    stats_path = OUTDIR / "prl_closure_sensitivity_gauge_marginalized_nu_ratio_stats.json"
    stats_path.write_text(json.dumps(stats, indent=2) + "\n")
    (ROOT_FIGDIR / "prl_closure_sensitivity_gauge_marginalized_nu_ratio_stats.json").write_text(
