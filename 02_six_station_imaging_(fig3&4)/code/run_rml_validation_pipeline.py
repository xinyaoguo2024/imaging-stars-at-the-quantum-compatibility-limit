...
from mixed_covariance import (
    mixed_covariance_cholesky,
    mixed_covariance_terms,
    mixed_quadratic_objective_and_gradient,
)
...
def chi2_target_score_values(values: dict[str, float]) -> float:
    ...
    amp_target = max(
        float(os.environ.get("RML_AMP_CHI2_TARGET", "1.0")), 1e-300
    )
    phase_target = max(float(os.environ.get("RML_PHASE_CHI2_TARGET", "1.0")), 1e-300)
    return float(
        max(
            abs(np.log(amp_chi2 / amp_target)),
            abs(np.log(phase_chi2 / phase_target)),
        )
    )
...
def cached_mixed_cholesky(
    ...
    """Cache the immutable mixed covariance factor across optimizer steps."""
    cache_key = f"_mixedchol_{strategy}_{float(phase_floor).hex()}"
    cached = band.get(cache_key)
    if cached is None:
        cached = mixed_covariance_cholesky(
            covariance,
            n_amplitude=n_amplitude,
            phase_floor=phase_floor,
        )
        band[cache_key] = cached
    return np.asarray(cached, dtype=float)


def local_closure_residual(
    band: dict[str, np.ndarray],
    strategy: str,
    model_phase: np.ndarray,
    data_phase: np.ndarray,
    closure_basis_matrix: np.ndarray,
) -> np.ndarray:
    """Return a locally unwrapped closure residual around the POVM work point.

    Finite-copy receiver covariances describe Gaussian fluctuations in the
    tangent space at a calibrated density matrix.  Mapping those coordinates
    back to independently principal-valued pseudo-edge phases destroys their
    correlated branch information.  New simulations therefore store the
    real-valued local closure estimate and its work-point reference directly.
    Legacy inputs retain the historical componentwise wrapped residual.
    """
    model_closure = model_phase @ closure_basis_matrix
    stored_data = band.get(f"closure_{strategy}")
    reference = band.get("closure_reference")
    if stored_data is None or reference is None:
        data_closure = data_phase @ closure_basis_matrix
        return np.angle(np.exp(1j * (model_closure - data_closure)))
    stored_data = np.asarray(stored_data, dtype=float).reshape(model_closure.shape)
    reference = np.asarray(reference, dtype=float).reshape(model_closure.shape)
    model_local = reference + np.angle(
        np.exp(1j * (model_closure - reference))
    )
    return model_local - stored_data
...
def residual_diagnostics(
    ...
    mixed_z_values: list[np.ndarray] = []
    for band in bands:
        model = base.interp_vis(vis_grid, uv_axis, band["u"], band["v"])
        amp_model = np.abs(model)
        amp_data = np.asarray(band["amp"], dtype=float)
        mixed_covariance = band.get(f"mixedcov_{strategy}")
        if mixed_covariance is not None:
            if strategy == "all":
                raise ValueError(
                    "mixedcov_all is not supported: mixed covariance ordering is "
                    "full-edge amplitude followed by closure coordinates"
                )
            if model.size % n_edges:
                raise ValueError(
                    f"band has {model.size} visibility samples, not a multiple of E={n_edges}"
                )
            n_sample = model.size // n_edges
            data = band[f"vis_{strategy}"]
            model_phase = np.angle(model).reshape(n_sample, n_edges)
            data_phase = np.angle(data).reshape(n_sample, n_edges)
            residual_amp = (amp_model - amp_data).reshape(n_sample, n_edges)
            residual_q = local_closure_residual(
                band, strategy, model_phase, data_phase, q_basis
            )
            residual_mixed = np.concatenate([residual_amp, residual_q], axis=1)
            mixed_cholesky = cached_mixed_cholesky(
                band,
                strategy,
                mixed_covariance,
                n_edges,
            ...
            mixed_z, _mixed_precision_residual = mixed_covariance_terms(
                residual_mixed,
                mixed_covariance,
                n_amplitude=n_edges,
                phase_floor=phase_floor,
                cholesky=mixed_cholesky,
            )
            amp_z_values.append(mixed_z[:, :n_edges].reshape(-1))
            phase_z_values.append(mixed_z[:, n_edges:].reshape(-1))
            mixed_z_values.append(mixed_z.reshape(-1))
            continue
        ...
        if strategy == "all":
            ...
            residual = local_closure_residual(
                band, strategy, model_phase, data_phase, q_basis
            )
    ...
    residual_arrays = {"amp_z": finite_amp, "phase_z": finite_phase}
    if mixed_z_values:
        finite_mixed = np.concatenate(mixed_z_values)
        finite_mixed = finite_mixed[np.isfinite(finite_mixed)]
        diagnostics["mixed_reduced_chi2"] = float(np.mean(finite_mixed**2))
        diagnostics["mixed_z_rms"] = float(np.sqrt(np.mean(finite_mixed**2)))
        diagnostics["n_mixed_samples"] = int(finite_mixed.size)
        residual_arrays["mixed_z"] = finite_mixed
    return diagnostics, residual_arrays
...
def physical_objective_and_grads(
    ...
    for band in bands:
        ...
        if mixed_covariance is not None:
            ...
            band_mixed_objective, residual_gradient, mixed_z = (
                mixed_quadratic_objective_and_gradient(
                    residual_mixed,
                    mixed_covariance,
                    n_amplitude=n_edges,
            ...
            mixed_data_objective += band_mixed_objective
            mixed_band_count += 1
            amp_chi2 += float(np.mean(mixed_z[:, :n_edges] ** 2))
            phase_chi2 += float(np.mean(mixed_z[:, n_edges:] ** 2))

            amp_residual_gradient = residual_gradient[:, :n_edges].reshape(-1)
            amp_coeff = amp_residual_gradient * model / np.maximum(amp_model, eps)
            amp_rml.bilinear_adjoint_samples(
                amp_grad_grid,
                uv_axis,
                band["u"],
                band["v"],
            ...
            closure_residual_gradient = residual_gradient[:, n_edges:]
            phase_coeff = (closure_residual_gradient @ q_basis.T).reshape(-1)
    ...
    if 0 < mixed_band_count < n_band:
        raise ValueError(
            f"mixedcov_{strategy} must be present for every band or for none; "
            f"found {mixed_band_count}/{n_band}"
        )
    using_mixed_covariance = mixed_band_count > 0
    if using_mixed_covariance:
        mixed_data_objective /= max(n_band, 1)
    amp_grad = amp_rml.adjoint_image(amp_grad_grid)
    phase_grad = amp_rml.adjoint_image(phase_grad_grid)
    if using_mixed_covariance:
        # Each band helper returns a reduced objective.  Match the arithmetic
        # mean over bands used by the scalar mixed likelihood.
        amp_grad /= max(n_band, 1)
        phase_grad /= max(n_band, 1)
    ...
    if using_mixed_covariance:
        total = mixed_data_objective + reg_value
        values = {
            "objective": float(total),
            "amp_chi2": float(amp_chi2),
            "phase_chi2": float(phase_chi2),
            "mixed_chi2": float(2.0 * mixed_data_objective),
        "reg": float(reg_value),
        }
        grads = {
            "amp": amp_grad,
            "phase": phase_grad,
            "data": amp_grad + phase_grad,
            "reg": reg_grad,
        }
    else:
...
def physical_amp_closure_rml(
    ...
    for iteration in range(PHYSICAL_ITER):
        ...
        if "data" in grads:
            # Cross-covariance makes separate amplitude/phase sign choices
            # meaningless: both blocks are parts of one Gaussian likelihood.
            data_dir = amp_rml.normalize_grad(grads["data"])
            directions = [
                (
                    f"{data_sign:+.0f}mixed",
                    amp_rml.normalize_grad(data_sign * data_dir + reg_dir),
                )
                for data_sign in (-1.0, 1.0)
            ]
        else:
...
def copy_band_with_rows(band: dict[str, np.ndarray], row_mask: np.ndarray, n_edges: int) -> dict[str, np.ndarray]:
    ...
    for key, value in band.items():
        ...
        if arr.ndim == 1 and arr.size == rows * n_edges:
            ...
            (key.startswith("mixedcov_") or key.startswith("_mixedchol_"))
            and arr.ndim == 3
            and arr.shape[0] == rows
        ):
            out[key] = arr[row_mask].copy()
        else:
            out[key] = arr.copy()
    return out
